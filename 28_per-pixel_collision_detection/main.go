...
func listen() {
	...
	var dot Dot = newDot(0, 0)
	var otherDot Dot = newDot(int(screenWidth)/4, int(screenHeight)/4)
	...
	for running {
		...
		updateRender(avgFPS, &dot, &otherDot)
...
// 更新处理
func updateRender(avgFPS float32, dot *Dot, otherDot *Dot) {
